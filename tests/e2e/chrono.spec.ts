...
test('asks for confirmation before cancelling a task already started', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('button', { name: 'Chronométrer une tâche' }).click()
  await page.getByRole('button', { name: 'Chronométrer Faire les litières' }).click()
  ...
  await page.getByRole('button', { name: 'Annuler' }).click()
  await expect(page.getByText('Abandonner ce chronométrage ?')).toBeVisible()

  await page.getByRole('button', { name: 'Continuer' }).click()
  await expect(page.getByText('Abandonner ce chronométrage ?')).toHaveCount(0)
